...
async function main() {
    ...
    try {
        ...
        serverProcess = spawn('node', ['packages/server/src/index.js'], {
            env: { ...process.env, PORT: BENCH_PORT, WS_PORT: 8089, AOF_FILE_PATH: 'benchmark.aof' },
        });

        // Use our new, simpler server checker
        await waitForServer(serverProcess);
        
        console.log('\n--- Running SET Benchmark ---');
        const setStats = await runBenchmark(['SET', 'mykey', `value-${'x'.repeat(100)}`]);
...
async function runWorker(command, histogram) {
    ...
    await new Promise(resolve => client.connect(BENCH_PORT, '127.0.0.1', resolve));
...
// --- Simplified Request/Response Helper ---
...
// --- Simplified Server Ready Checker ---
function waitForServer(serverProcess) {
    ...
    return new Promise((resolve, reject) => {
        ...
        serverProcess.on('exit', code => reject(`Server process exited with code ${code}`));
    });
}
...
function printStats(name, stats) {
    ...
    console.log(`  p50 (Median): ${(stats.histogram.getValueAtPercentile(50) / 1000).toFixed(3)}`);
    console.log(`  p90:          ${(stats.histogram.getValueAtPercentile(90) / 1000).toFixed(3)}`);
    console.log(`  p99:          ${(stats.histogram.getValueAtPercentile(99) / 1000).toFixed(3)}`);
}
