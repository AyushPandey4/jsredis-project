...
function start() {
  ...
  const server = net.createServer((socket) => {
    console.log(`Client connected on main port from: ${socket.remoteAddress}:${socket.remotePort}`);
  ...
  const replicationServer = net.createServer((socket) => {
    console.log(`Replica connected on replication port from: ${socket.remoteAddress}:${socket.remotePort}`);
    ...
    try {
      ...
      if (e.code !== 'ENOENT') { console.error('Error reading AOF for replica sync:', e); }
    }
    
    socket.on('close', () => {
      console.log(`Replica disconnected: ${socket.remoteAddress}:${socket.remotePort}`);
  ...
  replicationServer.listen(REPLICATION_PORT, '127.0.0.1', () => {
    console.log(`JSRedis Replication server is running on port ${REPLICATION_PORT}`);
  });
}
