...
// Listen for the metrics update from our new module
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
      ...
      try {
        ...
        ws.send(JSON.stringify({ type: 'response', payload: '-ERR Invalid JSON format\r\n' }));
        return;
      }
      ...
      switch (data.type) {
        ...
        case 'command':
          ...
          break;
        default:
          ws.send(JSON.stringify({ type: 'response', payload: '-ERR Unknown message type\r\n' }));
      }
    });
