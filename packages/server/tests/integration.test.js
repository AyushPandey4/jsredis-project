...
class TestClient {
        ...
        this.socket.on('data', (chunk) => {
            ...
            while (this.resolveQueue.length > 0) {
                ...
                // Handle bulk strings
                    ...
                    const length = parseInt(this.responseBuffer.slice(1, crlfIndex).toString(), 10);
                    if (length === -1) {
    ...
    connect() {
        return new Promise((resolve) => this.socket.connect(this.port, '127.0.0.1', resolve));
    }

    sendCommand(commandArray) {
        const respCommand = `*${commandArray.length}\r\n` + commandArray.map(arg => `$${Buffer.byteLength(arg)}\r\n${arg}\r\n`).join('');
    ...
    const startServer = () => {
      return new Promise((resolve) => {
          serverProcess = spawn('node', ['src/index.js'], {
              ...
              env: { ...process.env, PORT: TEST_PORT, WS_PORT: 8099, AOF_FILE_PATH: TEST_AOF_PATH },
          });
